...
interface ProgressFilterSearchProps {
    searchQuery?: string;
    ...
    selectedStatusTags?: number[];
    onStatusTagsChange?: (statusTags: number[]) => void;
    onClearFilters?: () => void;
}

export function ProgressFilterSearch({
    searchQuery = '',
    onSearchChange,
    selectedBlock = '',
    onBlockChange,
...
/* =======================
   PROGRESS FILTER MODAL
======================= */
interface ProgressFilterModalProps {
    block: string;
...
function ProgressFilterModal({
    ...
    onClose,
    onOpenSort,
}: ProgressFilterModalProps) {
    // Thesis stage tags
    const thesisStageTags = [
        'Title Proposal',
    ...
    // Status tags
    const statusOptions = [0, 1, 2]; // Pending Review, Approved, Denied
    ...
    const toggleStatusTag = (status: number) => {
        const newStatusTags = selectedStatusTags.includes(status)
            ? selectedStatusTags.filter((s) => s !== status)
            : [...selectedStatusTags, status];
        onStatusTagsChange(newStatusTags);
    };
    ...
    const handleApply = () => {
        console.log('Applied:', { block, selectedTags, selectedStatusTags });
        onClose();
    };
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                        {statusOptions.map((status) => (
                            <button
                                ...
                                className={cn(
                                    'rounded-lg border-[0.8px] px-3 py-1 text-[12px] font-medium transition-all duration-200',
                                    selectedStatusTags.includes(status)
                                        ? 'border-primary bg-primary text-white'
                                        : 'border-primary bg-white text-primary hover:bg-primary/5',
                                )}
                            >
